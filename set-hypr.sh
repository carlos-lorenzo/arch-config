...
if [[ $CFG == "Y" || $CFG == "y" ]]; then
    ...
    do 
        ...
        if [ -d "$DIRPATH" ]; then 
            ...
            cp $DIRPATH $DIRPATH-back &>> $INSTLOG
    ...
    read -rep $'[\e[1;33mACTION\e[0m] - Would you like to setup the login screen (y,n) ' LGNS
    if [[ $LGNS == "Y" || $LGNS == "y" ]]; then
...
# Copying cursor theme
echo -e "$CNT - Copying cursor theme..."
cp -r Extras/Dracula-cursors ~/.icons/ &>> $INSTLOG
